...
from nn_speedup_pisd_model import NN_PISD
import warnings
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_pos", default=2, type=int, help="Or You can just use dataset pos in the"
                                                                            "config.py file")
    parser.add_argument("--first_w", default=9, type=int, help="The first window size for tuning num_of_pip parameter")

    args = parser.parse_args()

    all_dataset_name = config.ALL_DATASET_NAME
    classifier_flag = "PISD-F"
    ...
    # w parameter
    init_w = [i for i in range(1, 11) if i != first_w]
    if len(init_w) == 0: init_w.append(1)
    list_w = np.flip((np.unique(init_w)))
    ...
    for no_pip in list_no_pip:
        ...
        print("fix w: %s - no_pip: %s/%s - c_best_train_error_rate: %s - best_noip: %s"
    ...
    if best_no_pip == 3:
        ...
        for w in list_w:
            ...
            print("fix no_pip: %s - w: %s/%s - c_best_train_error_rate: %s - best_w: %s"
                  % (best_no_pip, w, list_w[-1], best_train_error_rate, best_w))

    print("best no_pip: %s - best w: %s - best_train_error_rate: %s" % (best_no_pip, best_w, best_train_error_rate))
    ...
    print("dts: %s - no_pip: %s - w: %s - train_acc: %s - test_acc: %s"
